...
with DAG(
    ...
    def get_datetime_calc(**kwargs):
        # 스케줄러 부하 경감을 위해 task decorator안에 선언
        # 스케줄러가 주기적으로 dag 파싱, with DAG 전 / as dag 밑 operator 선언 전에 작성 시 dag이 실행되지 않아도 주기적으로 검사, 부하 커짐
        # operator, task decorator 안에 작성 시 검사 X
